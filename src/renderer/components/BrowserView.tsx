...
import { isInternalUrl, SETTINGS_URL, AI_CHAT_URL, START_PAGE_URL, type Tab } from '../hooks/useBrowserTabs';
...
function WebviewSlot({
  ...
  return (
    ...
    <webview ref={setRef} className="webview" style={{ display: isActive ? 'flex' : 'none' }} />
  );
}

export default function BrowserView({ tabs, activeTabId, registerWebviewRef, bindWebviewEvents }: Props) {
  return (
    <div className="browser-container">
      {tabs.map((tab) => {
        ...
        if (isInternalUrl(tab.url)) {
          ...
          return (
            <div key={tab.id} className="internal-page" style={{ display: isActive ? 'flex' : 'none' }}>
              {tab.url === SETTINGS_URL && <SettingsPage />}
              {tab.url === AI_CHAT_URL && <AIChatPage />}
              {tab.url === START_PAGE_URL && <StartPage />}
            </div>
          );
        }
